...
addon = xbmcaddon.Addon(id='script.module.mixedsports')
...
def Main_Menu():
    menu=[
        ['[COLORgoldenrod] - - - Click here for Mixed Live Sports Events - - -[/COLOR]','sched'],
    ]
    for m in menu:
...
def TransList(categ):
    ...
    for t in trns:
        ...
        if len(t[1])==1:
            tr = t[1][0] 
            tr = 'https://daddylivehd.com/' + tr if tr.startswith('/') else tr
...
def getSource(trData):
    ...
    if select > -1:
        ...
        url_stream = 'https://daddylivehd.com/' + url_stream if url_stream.startswith('/') else url_stream
        print(url_stream)
