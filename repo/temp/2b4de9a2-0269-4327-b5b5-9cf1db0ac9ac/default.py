...
INCLUDEfamilyeasybuild    = wiz.getS('includefamilyeasybuild')
INCLUDEHOMELANDER    = wiz.getS('includehomelander')
INCLUDEMORIA    = wiz.getS('includemoria')
...
DEFAULTPLUGINS   = ['game.controller.default', 'game.controller.snes', 'metadata.album.universal', 'metadata.artists.universal', 'metadata.common.fanart.tv', 'metadata.common.imdb.com', 'metadata.common.themoviedb.org', 'metadata.themoviedb.org.python', 'metadata.tvshows.themoviedb.org', 'metadata.tvshows.themoviedb.org.python', 'service.xbmc.versioncheck']
try:
...
def maintMenu(view=None):
    ...
    if includeall == 'true':
        includeseren = 'true'
        includegaia = 'true'
        includefamilyeasybuild = 'true'
        includehomelander = 'true'
        includemoria = 'true'
    else:
        includefamilyeasybuild = 'true' if INCLUDEfamilyeasybuild     == 'true' else 'false'
    ...
    if includevid == 'true':
        ...
        if xbmc.getCondVisibility('System.HasAddon(plugin.video.family easybuild)'): addFile('--- Include family easybuild: %s' % includefamilyeasybuild.replace('true',on).replace('false',off), 'togglecache', 'includefamilyeasybuild', icon=ICONMAINT, themeit=THEME3)
...
def toggleCache(state):
    cachelist = ['includevideo', 'includeall', 'includegaia', 'includefamilyeasybuild', 'includeseren']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Gaia', 'Include family easybuild', 'Include Homelander', 'Include Moria', 'Include Seren']
